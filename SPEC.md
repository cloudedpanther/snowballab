...
- **전제**: 연 수익률 6%를 “월 (6/12)% = 0.5%”로 적용. 이자는 **복리가 아니라 단리**: 각 금액이 계좌에 머문 **개월 수 × 0.5%**만큼만 이자를 받는다.
- **1년차**
  - 원금: 초기금 + 당해 연도에 넣은 적립금. 1월에는 초기금만 있고, 2월~12월 초에 매월 1회씩 적립하므로 **원금 = 초기금 + (적립금 × 11)**.
  - 수익: 각 “덩어리”가 머문 개월 수만큼만 0.5% 적용.
    - 초기금 100,000원: 12개월 → 100,000 × 0.005 × 12
    - 1월에 넣은 100,000원: 11개월 → 100,000 × 0.005 × 11
    - … 11월에 넣은 100,000원: 1개월 → 100,000 × 0.005 × 1
  - 합계: **수익 = 100,000 × 0.005 × (12+11+…+1) = 100,000 × 0.005 × 78 = 39,000원**.
  - **1년차 최종 = 원금 1,200,000 + 수익 39,000 = 1,239,000원**.
- **2년차**
  - 원금: 전년 말 잔액(1,239,000) + 당해 연도 적립금 12회분 = 1,239,000 + 1,200,000 = **2,439,000원**.
  - 수익: (전년 말 잔액 × 0.005 × 12) + (당해 연도 적립금에 대한 단리)  
    = 1,239,000 × 0.005 × 12 + 100,000 × 0.005 × (12+11+…+1)  
    = 74,340 + 39,000 = **113,340원**.
  - **2년차 최종 = 2,439,000 + 113,340 = 2,552,340원**.
- **일반(연도 k ≥ 2)**
  - 원금\_k = (연도 k-1 최종 잔액) + (당해 연도 적립금 12회분).
  - 수익\_k = (연도 k-1 최종 잔액) × 0.005 × 12 + (적립금 × 0.005 × 78).
  - 최종\_k = 원금\_k + 수익\_k.
- **월별 보기**: 연도별과 동일한 결과가 나오도록, “연도 경계에서 잔액을 마감하고, 당해 연도에는 전년 말 잔액(올해 들어온 금액) + 당해 연도 납입분”만으로 당해 연도 이자를 계산. 즉 연도 y의 월 t 말 잔액 = (전년 말 잔액)×(1+0.005×t) + (당해 연도 납입 t회)×(원금) + (당해 연도 납입 t회에 대한 단리). 반올림은 계산 단계에서는 하지 않고, 사용자에게 보여줄 때만 적용.
